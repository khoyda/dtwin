"""Run the spring-wheat process model on real ECCC weather for a season.

    python scripts/run_wheat_model.py [year]
"""
...
def main() -> None:
    ...
    print(f"== Spring-wheat model — {info['name']} (SK), {year} ==")
    result = WheatCropModel(WheatParameters.from_calibrated(cfg)).run(s, float(info["lat"]))
    ...
    for stage, grp in result.daily.groupby("stage"):
        ...
        print(f"   {WheatStage(int(stage)).name:<11} {grp['date'].iloc[0].date()} (day {d})")
