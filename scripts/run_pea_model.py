"""Run the yellow-pea process model on real ECCC weather for a season.

    python scripts/run_pea_model.py [year]
"""
...
def main() -> None:
    ...
    print(f"== Yellow-pea model — {info['name']} (SK), {year} ==")
    result = PeaCropModel(PeaParameters.from_calibrated(cfg)).run(s, float(info["lat"]))
    ...
    for stage, grp in result.daily.groupby("stage"):
        ...
        print(f"   {PeaStage(int(stage)).name:<11} {grp['date'].iloc[0].date()} (day {d})")
